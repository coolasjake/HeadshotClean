...
public class fts {
    ...
    /// <summary>
    /// Calculate the desired starting velocity of a projectile so that it follows an arc and hits the target while moving laterally at a predefined speed.
    /// </summary>
    ...
    /// <returns></returns>
    public static bool solve_ballistic_arc_lateral(Vector3 proj_pos, float lateral_speed, float gravity, Vector3 target, Vector3 target_velocity, out Vector3 fire_velocity)
    {
        ...
        float peak_time = t / 2f;
        float max_height_offset = (Mathf.Abs(gravity) * (peak_time * peak_time)) / 2f;
        max_height_offset -= Mathf.Abs(height_diff) / 2f;
